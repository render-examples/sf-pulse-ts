...
describe("GET /api/restaurants/needing-menu-check", () => {
  ...
  it("returns restaurants without 'upcoming' that have no menu_checked_at", async () => {
    ...
    assert.equal(res.status, 200);
    assert.ok(Array.isArray(res.body));
    // The seed data has restaurants with and without "upcoming" in opened_date.
    // All non-upcoming ones should be returned since menu_checked_at is null.
    const names = res.body.map((r: { name: string }) => r.name);
    assert.ok(names.length > 0, "should return some restaurants");
    // Upcoming restaurants should not be in the list
