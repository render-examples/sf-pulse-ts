...
function restaurantCompletenessScore(restaurant: Pick<Restaurant, "neighborhood" | "cuisine" | "address" | "opened_date">): number {
  return [
    ...
    DATE_PRECISION_SCORE[getDatePrecision(stripRestaurantDateQualifier(restaurant.opened_date))],
  ].reduce((sum, value) => sum + value, 0);
}

function eventCompletenessScore(event: Pick<Event, "location" | "date" | "time" | "description">): number {
  return [
...
function mergeRestaurant(base: Restaurant, patch: Partial<NewRestaurant>): NewRestaurant {
  const name = patch.name ? normalizeWhitespace(decodeHtmlEntitiesRecursive(patch.name)) : base.name;
  const neighborhood = patch.neighborhood
    ? normalizeWhitespace(patch.neighborhood)
    : base.neighborhood;
  const cuisine = patch.cuisine ? normalizeWhitespace(patch.cuisine) : base.cuisine;
...
function mergeEvent(base: Event, patch: Partial<NewEvent>): NewEvent {
  return {
    title: patch.title ? normalizeEscapedHtmlText(patch.title) : normalizeEscapedHtmlText(base.title),
    ...
    date: patch.date ? normalizeDateText(patch.date) : normalizeDateText(base.date),
...
function similarRestaurantIdentity(left: Pick<Restaurant, "name" | "address">, right: Pick<Restaurant, "name" | "address">): boolean {
...
function desiredEventKey(event: Pick<Event | NewEvent, "title" | "location" | "date">): string {
  return [
...
function isDefinitelyNonSanFranciscoEvent(event: Pick<Event, "title" | "source_url">): boolean {
...
async function enrichRestaurant(restaurant: Restaurant): Promise<RestaurantDecision> {
  let next = mergeRestaurant(restaurant, {});
  const sourceUrl = restaurant.source_url ?? "";

  if (restaurant.opened_date.includes("(upcoming)")) {
    next = mergeRestaurant(restaurant, { ...next, opened_date: restaurant.opened_date });
  }

  if (sourceUrl.includes("guide.michelin.com/us/en/california/san-francisco/restaurant/")) {
    ...
    if (details) {
      next = mergeRestaurant(restaurant, {
        ...
        cuisine:
          restaurant.cuisine.toLowerCase().includes("michelin")
            ? details.cuisine
            : next.cuisine,
      });
    }
  ...
  if (sourceUrl.includes("sf.eater.com/restaurant-news/")) {
    const html = await fetchPageHtml(sourceUrl);
    const candidates = html ? parseEaterArticle(html, [], sourceUrl, next.opened_date) : [];
    ...
    if (match) {
      ...
      return { action: "delete", reason: "Out-of-scope or malformed Eater import" };
    }
  }
...
async function enrichEvent(event: Event): Promise<EventDecision> {
  ...
  if (sourceUrl.startsWith("https://sf.funcheap.com/") && !sourceUrl.includes("/city-guide/")) {
    const html = await fetchPageHtml(sourceUrl);
...
async function main(): Promise<void> {
  ...
  for (const restaurant of restaurants) {
    ...
    if (
      ...
      normalizeRestaurantOpenedDateText(restaurant.opened_date) !== next.opened_date
    ) {
  ...
  for (const left of restaurantRows) {
    ...
    for (const right of restaurantRows) {
      ...
      const keepLeft =
        restaurantCompletenessScore(left.row) >= restaurantCompletenessScore(right.row);
      const keeper = keepLeft ? left : right;
      const loser = keepLeft ? right : left;
      restaurantDeletes.set(loser.id, "Duplicate restaurant after normalization");
      restaurantUpdates.set(
        keeper.id,
        mergeRestaurant(keeper.row, {
          name: keeper.row.name.length >= loser.row.name.length ? keeper.row.name : loser.row.name,
          neighborhood:
          ...
          opened_date:
            DATE_PRECISION_SCORE[getDatePrecision(stripRestaurantDateQualifier(keeper.row.opened_date))] >=
            DATE_PRECISION_SCORE[getDatePrecision(stripRestaurantDateQualifier(loser.row.opened_date))]
  ...
  for (const event of events) {
    ...
    if (
      ...
      (stripFuncheapAttribution(event.description) ?? null) !== (next.description ?? null)
    ) {
  ...
  const candidateEvents = events
    ...
    .map((event) => ({
      id: event.id,
      row: eventUpdates.get(event.id) ? { ...event, ...eventUpdates.get(event.id)! } : event,
    }));
  ...
  for (const group of eventGroups.values()) {
    ...
    group.sort((left, right) => eventCompletenessScore(right.row) - eventCompletenessScore(left.row) || left.id - right.id);
      ...
      mergeEvent(keeper.row, {
        location:
          keeper.row.location.toLowerCase() === "san francisco"
            ? group.find((candidate) => candidate.row.location.toLowerCase() !== "san francisco")?.row.location
            : keeper.row.location,
        description:
          group
            .map((candidate) => candidate.row.description)
            .filter((value): value is string => Boolean(value))
            .sort((left, right) => right.length - left.length)[0] ?? keeper.row.description,
        time: keeper.row.time ?? group.find((candidate) => candidate.row.time)?.row.time ?? null,
      }),
    );
  }
  ...
  if (!ALLOW_DELETE && (restaurantDeletes.size > 0 || eventDeletes.size > 0)) {
    console.warn("Delete candidates found; rerun with --allow-delete to remove them.");
    return;
  }
